...
class Suite15Skills extends BaseTest {
    ...
    void test_skill_script_discovery_includes_filename(@TempDir Path dir) throws Exception {
        ...
        Map<String, Map<String, String>> noScriptsCfg = (Map<String, Map<String, String>>)
    ...
    void test_skill_per_sub_agent_model_override(@TempDir Path dir) throws Exception {
        ...
        Map<String, String> threaded = (Map<String, String>) cfg.getOrDefault("agentModels", cfg.get("agent_models"));
        assertNotNull(
                threaded,
        ...
        Map<String, String> defaultThreaded =
                (Map<String, String>) defaultCfg.getOrDefault("agentModels", defaultCfg.get("agent_models"));
        assertTrue(
                defaultThreaded == null || defaultThreaded.isEmpty(),
    ...
    void test_skill_params_and_cross_skill_refs_are_threaded(@TempDir Path parent) throws Exception {
        ...
        Files.writeString(
                main.resolve("SKILL.md"),
                "---\nname: main-skill\nparams:\n  mode:\n    default: fast\n---\n# Main\nUse the child-skill skill.\n");
        Files.writeString(
                child.resolve("SKILL.md"),
        ...
        assertTrue(
                nestedRefs.containsKey("grandchild-skill"),
        ...
        assertTrue(
                isolatedRefs.containsKey("isolated-child"),
    ...
    void test_skill_nested_in_agent_tool_compiles(@TempDir Path dir) throws Exception {
        ...
        assertTrue(
                workerNamesObj instanceof List,
        ...
        assertTrue(
                wfStr.contains("test_skill_e2e_s17"),
    ...
    void test_standalone_skill_script_runs_as_worker_tool(@TempDir Path dir) throws Exception {
        ...
        assertTrue(
                result.isSuccess(),
                "Standalone skill run must complete. executionId=" + result.getExecutionId() + " status="
    ...
    void test_agent_tool_skill_workers_with_domain(@TempDir Path dir) throws Exception {
        ...
        Agent parent = Agent.builder()
                ...
                .model(MODEL)
                .instructions(
                        "You have one tool: test_skill_e2e_s17. Call it once with the user's request, then return the result.")
                .tools(List.of(skillTool))
                .stateful(true)
                .maxTurns(3)
                .build();
        ...
        assertTrue(
                result.isSuccess(),
                "Nested stateful skill run must complete. executionId=" + result.getExecutionId() + " status="
                        + result.getStatus() + " error=" + result.getError());
        ...
        Map<String, Object> skillTask = ((List<Map<String, Object>>) tasksObj)
                .stream()
        ...
        verifyWorkerTask(subWorkflow, "test_skill_e2e_s17__echo_args", "ECHO_ARGS_RESULT:");
    }
    ...
    private static void verifyWorkerTask(Map<String, Object> workflow, String taskName, String marker) {
        ...
        assertFalse(
                matches.isEmpty(),
                taskName + " was not invoked. Task defs: "
                        + tasks.stream().map(t -> t.get("taskDefName")).toList());
        for (Map<String, Object> task : matches) {
            assertEquals("COMPLETED", task.get("status"), taskName + " must complete as a worker task. Task: " + task);
        }
