...
class Suite18ToolTypes extends BaseTest {
    ...
    private AgentResult runWith(String agentName, String prompt) {
        ...
        Agent agent = Agent.builder()
                ...
                .model(MODEL)
                .instructions(
                        "You must call the specified tool with the values from the user message exactly as given. "
    ...
    void test_local_date_is_local_date() {
        ...
        AgentResult result = runWith("e2e_java_types_localdate", "Call record_item_date with date=2026-05-12.");

        assertEquals(AgentStatus.COMPLETED, result.getStatus(), "agent did not complete. error=" + result.getError());
        ...
        assertInstanceOf(LocalDate.class, seen, "LocalDate parameter received as " + seen.getClass());
    ...
    void test_instant_is_instant() {
        ...
        AgentResult result =
                runWith("e2e_java_types_instant", "Call record_event_time with when=2026-05-12T13:45:00Z.");

        assertEquals(AgentStatus.COMPLETED, result.getStatus(), "agent did not complete. error=" + result.getError());
        Object seen = TypeTools.seenInstant.get();
        assertNotNull(seen);
        assertInstanceOf(Instant.class, seen, "Instant parameter received as " + seen.getClass());
    }
    ...
    void test_list_of_local_date_elements_are_local_dates() {
        ...
        AgentResult result = runWith(
                "e2e_java_types_listdate", "Call record_item_dates with dates=[\"2026-05-12\", \"2026-05-13\"].");
