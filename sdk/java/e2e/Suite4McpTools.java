...
class Suite4McpTools extends BaseTest {
    ...
    void test_mcp_tool_def_basic_properties() {
        ...
        assertEquals("e2e_s14_basic", mcp.getName(), "MCP tool name must round-trip.");
        assertEquals("Basic MCP tool", mcp.getDescription(), "MCP tool description must round-trip.");
        assertEquals(
                "mcp",
                mcp.getToolType(),
    ...
    void test_mcp_tool_credentials_round_trip() {
        ...
        assertEquals(1, creds.size(), "Expected exactly 1 credential. Got: " + creds);
        assertEquals("MCP_AUTH_KEY", creds.get(0), "Credential should be 'MCP_AUTH_KEY'. Got: " + creds.get(0));
        ...
        ToolDef unauthed = McpTool.builder()
                ...
                .build();
        assertTrue(
                unauthed.getCredentials() == null || unauthed.getCredentials().isEmpty(),
    ...
    void test_mcp_credentials_nested_in_plan_config() {
        ...
        assertEquals(1, creds.size(), "Should have exactly one credential. Got: " + creds);
        assertEquals("MCP_AUTH_KEY", creds.get(0), "Credential name should be 'MCP_AUTH_KEY'. Got: " + creds.get(0));
