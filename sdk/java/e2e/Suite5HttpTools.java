...
class Suite5HttpTools extends BaseTest {
    ...
    void test_http_tool_def_basic_properties() {
        ...
        assertEquals("e2e_s15_basic", http.getName(), "HTTP tool name must round-trip.");
        assertEquals(
                "http",
                http.getToolType(),
    ...
    void test_http_tool_serializes_to_plan() {
        ...
        assertEquals("http", tool.get("toolType"), "Plan toolType should be 'http'. Got: " + tool.get("toolType"));
    ...
    void test_http_credentials_nested_in_plan_config() {
        ...
        assertEquals(1, creds.size(), "Expected 1 credential. Got: " + creds);
        assertEquals("HTTP_AUTH_KEY", creds.get(0), "Credential should be 'HTTP_AUTH_KEY'. Got: " + creds.get(0));
