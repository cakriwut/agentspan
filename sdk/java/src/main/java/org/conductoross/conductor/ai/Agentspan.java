...
public final class Agentspan {
    ...
    public static void configure(AgentConfig config) {
        synchronized (lock) {
            if (defaultRuntime != null) {
                throw new IllegalStateException("configure() must be called before the first run/start/stream call. "
    ...
    public static CompletableFuture<AgentResult> runAsync(
    ...
    public static AgentHandle start(
            org.bsc.langgraph4j.agentexecutor.AgentExecutor.Builder builder, String prompt, Object... tools) {
     ...
     *       {@link org.conductoross.conductor.ai.frameworks.AdkBridge} (ADK must be on the
